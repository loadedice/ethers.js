...
export class Wallet extends AbstractSigner {
     ...
     */
    public static createRandom(options?: any): Wallet {
    ...
    public static fromEncryptedJson(json: string, password: Arrayish, progressCallback?: ProgressCallback): Promise<Wallet> {
        if (isCrowdsaleWallet(json)) {
            try {
                if (progressCallback) { progressCallback(0); }
                const privateKey = secretStorage.decryptCrowdsale(json, password);
    ...
    public static fromMnemonic(mnemonic: string, path?: string, wordlist?: Wordlist): Wallet {
     ...
     */
    public connect(provider: Provider): Wallet {
    ...
    public sign(transaction: TransactionRequest): Promise<string> {
        return resolveProperties(transaction).then((tx) => {
            const rawTx = serializeTransaction(tx);
            const signature = this.signingKey.signDigest(keccak256(rawTx));
    ...
    public encrypt(password: Arrayish | string, options?: any, progressCallback?: ProgressCallback): Promise<string> {
